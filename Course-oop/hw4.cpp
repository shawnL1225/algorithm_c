...
class SDN_stat_payload : public payload {
    ...
    public:
        ...
        // SET(setActID,unsigned int,actID,_actID);
        // GET(getActID,unsigned int,actID);
        vector<pair<int,int>> getStatInfo() const { return stat_info;}
        void setStatInfo( vector<pair<int,int>> _stat_info) {stat_info = _stat_info;}
...
class SDN_switch: public node {
    ...
    public:
        map<unsigned int, unsigned int> next_table;
        ~SDN_switch(){}
...
void stat_packet_event (unsigned int con_id, unsigned int id, vector<pair<int,int>> weight_info, unsigned int t = event::getCurTime(), string msg = "default"){
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet" ) { // the switch receives a packet from the other switch
        ...
        int dst = p2->getHeader()->getDstID();

        if( dst == getNodeID()) return;
        if( !isSetCtrl ) return; //if havn't set routing table
        if(next_table.count(dst) == 0) return; // if the dstination is not in the dst set
        p2->getHeader()->setPreID ( getNodeID());
        p2->getHeader()->setNexID (next_table[dst]);
        p2->getHeader()->setDstID ( dst );
        // cout << "node " << getNodeID() << " send the packet"<<pid << " to :" << next_table[dst] << endl;
        send_handler (p2);
    }
        ...
        l3->setMsg("ack"); //send ack to controller
        int src = p3->getHeader()->getSrcID();
        int dst = p3->getHeader()->getDstID();
        p3->getHeader()->setDstID(src); //set package info
        p3->getHeader()->setSrcID(dst);
        p3->getHeader()->setNexID(src);
        p3->getHeader()->setPreID(getNodeID());
        
        next_table[l3->getMatID()] = l3->getActID();
        isSetCtrl = 1;
        send_handler(p3);
    }
...
#define S_INIT_ACK 2
#define S_UPDATE 3
#define S_UPDATE_ACK 4
usint nodesNum, dstNum, linkNum;
usint InsTime, UpdTime, SimDuration;
vector<usint> dstId;
class Link {
  public:
  int node1,node2;
  int oldw, neww;
  Link(){}
  Link(int n1,int n2,int w1, int w2) : node1(n1), node2(n2), oldw(w1), neww(w2){}
...
vector<Link> links; //double direction
vector<Node> nodes;
struct ctrlPackage {
    int round;
    int id;
...
bool cmp(ctrlPackage a, ctrlPackage b){
    return a.round<b.round;
}
deque<ctrlPackage> roundOrder[50]; //sort ctrl pkg by round
map<usint,usint> dstOrderMap; //dst to the indexOfDst
...
void SDN_controller::recv_handler (packet *p){
    ...
    if(p->type() == "SDN_ctrl_packet"){
        ...
        int dst = p3->getHeader()->getDstID();
        int src = p3->getHeader()->getSrcID();
        int prev = p3->getHeader()->getPreID();
        int mat = l3->getMatID();
        int whichDst=dstOrderMap[mat];
        if(dst != getNodeID()){ //send to switch
            
            state[whichDst][dst]++; //update state
            p3->getHeader()->setNexID(dst);
            p3->getHeader()->setPreID(getNodeID()); 
            send_handler(p3);
        }else if(l3->getMsg() == "ack" ){ //get ack from switch
            
            state[whichDst][src]++; //ack state
            if(state[whichDst][src] == 2) return;
            bool finish = true;
            for(usint i=0;i<nodesNum;i++){ //check whether all ack
                if(state[whichDst][i] == 3) finish = false;
            }   
            
            if(finish && !roundOrder[whichDst].empty()){ //send next round pkg
                ctrlPackage P;
                do{ //get same round pkg
                    P = roundOrder[whichDst][0];
                    roundOrder[whichDst].pop_front();
                    ctrl_packet_event(nodesNum, P.id, P.mat, P.act);
                    // printf("ctrl update %d \n", P.id);
                } while(!roundOrder[whichDst].empty() && roundOrder[whichDst][0].round == P.round);
        ...
        int dst = p3->getHeader()->getDstID();
        int src = p3->getHeader()->getSrcID();
        vector<pair<int,int>> link_info = l3->getStatInfo();
        for(auto link : link_info){
            links.push_back(Link(src,link.first,link.second,link.second));
            links.push_back(Link(link.first,src,link.second,link.second));
        }
        recv_stat[src] ++;

        if(!installed){ //for install
            //check whether recieve all
            for(usint i=0;i<nodesNum;i++){ 
                if(recv_stat[i] == 0) return;
            } 
            //build install(old weight) tree with bellman ford 
            ...
            //init old routing table
            for(auto n:nodes){
                for(auto d : dstId){
                    if(n.id == d) continue;
                    ctrl_packet_event(nodesNum, n.id, d, n.next_old[d]);
                    // cout <<n.id << ": "<< to_string(d) + " " + to_string(n.next_old[d]) << endl; 
                }
            }
            installed = 1;
            links.clear();
        }
        else if(installed){ //for update

            //check whether recieve all
            for(usint i=0;i<nodesNum;i++){ 
                if(recv_stat[i] == 1) return;
            } 

            //build update(new weight) tree with bellman ford 
            int dis[dstNum][nodesNum];
            usint round[dstNum][nodesNum];  //store update round
            set<usint> tree[dstNum][nodesNum]; //set up SPT
            for(int i=0;i<dstNum;i++){
                ...
                for(int k=0;k<nodesNum-1;k++){
                    ...
                    for(auto l :links){
                        ...
                        if(dis[i][l.node2] > dis[i][l.node1] + l.neww){
                            if(dis[i][l.node2] != 0x3f3f3f3f) {
                                tree[i][nodes[l.node2].next_new[dstId[i]]].erase(l.node2);//erase original parent
                                // printf("del %d %d",nodes[l.node2].next_new[dstId[i]], l.node2 ); 
                            }
                            tree[i][l.node1].insert(l.node2); //insert child
                            dis[i][l.node2] = dis[i][l.node1] + l.neww;
                            // printf("ins %d %d",l.node1, l.node2);
                        nodes[l.node2].next_new[dstId[i]] = l.node1;//upadate parent (routing table)
            ...
            //traverse tree with BFS to get round
            for(int i=0;i<dstNum;i++){
                queue<usint> Q;
                Q.push(dstId[i]);
                round[i][dstId[i]] = 0;
                while(!Q.empty()){
                    int front=Q.front();
                    Q.pop();
                    for(auto it=tree[i][front].begin(); it!=tree[i][front].end();it++){
                        round[i][*it] = round[i][front]+1;
                        Q.push(*it);
                        // cout << *it << ' '<< round[i][*it] << endl;
                    }
                }
            ...
            for(auto n : nodes){
                ...
                for(auto d : dstId){
                    ...
                    roundOrder[i].push_back({round[i][n.id],n.id,d,n.next_new[d]}); //push ctrl pkg to deque
                    // printf("update %d in round %d\n", n.id, round[i][n.id]);
                    i++;
                }
            }

            for(int i=0;i<dstNum;i++){ //sort ctrl pkg and send first round to controller
                sort(roundOrder[i].begin(),roundOrder[i].end(),cmp);
                ctrlPackage P;
                if( roundOrder[i].empty())continue; //loop to get first round
                do{
                    P = roundOrder[i][0];
                    roundOrder[i].pop_front();
                    ctrl_packet_event(nodesNum, P.id, P.mat, P.act);
                    // printf("ctrl update %d in round %d\n", P.id, 1);
                } while(!roundOrder[i].empty() && roundOrder[i][0].round == P.round);
            }
        }
...
int main()
{
    ...
    links.resize(linkNum*2);
    nodes.resize(nodesNum);
    unsigned int con_id = nodesNum;
    node::node_generator::generate("SDN_controller",con_id);
    //init node id
    for (unsigned int id = 0; id < nodesNum; id ++){
    ...
    for(int i=0;i<dstNum;i++){
        ...
        dstOrderMap[dst_num] = i;
    } 
    //input link
    int lid;
    vector<pair<int,int>> link_info_old[nodesNum], link_info_new[nodesNum];
    for(int i=0;i<linkNum;i++){
        cin >> lid >> links[i].node1 >> links[i].node2 >> links[i].oldw >> links[i].neww;
        node::id_to_node(links[i].node1)->add_phy_neighbor(links[i].node2);
        link_info_old[links[i].node1].push_back(make_pair(links[i].node2, links[i].oldw));
        link_info_new[links[i].node1].push_back(make_pair(links[i].node2, links[i].neww));
    }
    for(int i=lid+1;i<linkNum*2;i++){ //input another direction
     ...
     links.clear();
    //stat event send to switch
    for(int i=0; i<nodesNum; i++){
        stat_packet_event(con_id, i, link_info_old[i], InsTime);
        stat_packet_event(con_id, i, link_info_new[i], UpdTime);
    }
...
//             ctrl_packet_event(con_id, n.id, d, n.next_old[d], InsTime);
//             // cout <<n.id << ": "<< to_string(d) + " " + to_string(n.next_old[d]) << endl; 
//         }
//     }
...
//             P = roundOrder[i][0];
//             roundOrder[i].pop_front();
//             ctrl_packet_event(con_id, P.id, P.mat, P.act, UpdTime);
//             // printf("ctrl update %d in round %d\n", P.id, 1);
//         } while(!roundOrder[i].empty() && roundOrder[i][0].round == P.round);
//     }
  
    //set data packet
    unsigned int t = 0, src = 0, dst;
    while(cin >> t >> src >> dst){
