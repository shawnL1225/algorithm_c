...
int dot,Dest,Link; //dot表示點的個數 Dest表示幾個目的地 Link表示有幾條邊
map<int,int> rps[10000];
vector<int> DestId; //存放邊長的編號
class nodes{
        public:
            unsigned int LinkId,Node1,Node2,oldW,newW;
    }n[10000];

# define INF 0x3f3f3f3f
typedef long long int ll;
typedef pair<ll, ll> iPair;
map<unsigned int,unsigned int> old_weight[10000]; //存放舊的權重
map<unsigned int,unsigned int> new_weight[10000]; //存放新的權重
vector<unsigned int> vertex[10000];  //adjacency_matrix
map<unsigned int,unsigned int> old_table[10000]; //舊的table記錄舊的輸出資料
map<unsigned int,unsigned int> new_table[10000]; //新的table記錄新的輸出資料
int m,x,y,k,p,q;
...
class SDN_controller: public node {
        ...
        bool ini;
        bool hi; // this is used for example; you can remove it when doing hw2
        vector<int> cal[10000]; //紀錄該終點到任意點的狀態
...
void SDN_controller::recv_handler (packet *p){
    ...
    //cout << "ggggggg\n";
        unsigned int pre = p4->getHeader()->getPreID();
        unsigned int mat = l4->getMatID();
        unsigned int act = l4->getActID();
        unsigned int dst = p4->getHeader()->getDstID();
        unsigned int nex = p4->getHeader()->getNexID();
        unsigned int src = p4->getHeader()->getSrcID();
        //unsigned int pre = p4->getHeader()->getPreID();
        //cout << dst << " " << nex << " " << pre << '\n';
        
        if(dst == node::getNodeNum()){
            if(cal[dst][pre] == 1){
                cal[dst][pre] = 2;
            }
            //cout << "aaaaaaaaaa\n";
             //cal[dst][pre] = 2; //終點是controller將狀態改為2

             for(int i=0;i<node::getNodeNum()-1;i++){
                 if(cal[dst][i] == 1){
                     return;  //等於1這些封包還沒回來
                 }
             }
             ...
             for(int i=0;i<Dest;i++){
                 for(int j=0;j<node::getNodeNum()-1;j++){
                     if(rps[DestId[i]][j] == rps[DestId[i]][pre] + 1){
                         ctrl_packet_event(node::getNodeNum()-1, i, DestId[j], new_table[i][DestId[j]]); //根據bfs結果依序送出封包
                     }
                 }
             }
         }
             ...
             if(cal[dst][pre] == 0){
                 cal[dst][pre] = 1; //終點不是controller還須將他送出
                 p4->getHeader()->setNexID(dst);
                 send_handler(p4);
                 //cout << "hhhhhhh\n";
             }
        }
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet"){ // the switch receives a packet from the other switch
        ...
        p2->getHeader()->setPreID ( getNodeID() );  //p2存有當前點的值

        if(p2->getHeader()->getPreID()==p2->getHeader()->getDstID()){
            return ;
        }
        else{
            map<int,int>::iterator it;
            it = table.find(p2->getHeader()->getDstID());  //判斷p2是否存在table裡面如果不存在就返回
            if(it == table.end()){
                return ;
            }
        }
        p2->getHeader()->setNexID ( table[p2->getHeader()->getDstID()] );  //更新下一個點
        //p2->getHeader()->setDstID ( getDstID() );
        //hi = true;
        send_handler (p2);
    }
        ...
        if(table[mat] != act){
            table[mat] = act;  //table裡的點和act不同 就將它更新
        }



        unsigned int temp_Dst = p3->getHeader() -> getDstID();
        unsigned int temp_Src = p3->getHeader()-> getSrcID();
        p3->getHeader()->setSrcID(temp_Dst);
        p3->getHeader()->setDstID(temp_Src); //src和dst對調

        unsigned int temp_Pre = p3->getHeader() -> getPreID();
        unsigned int temp_Nex = p3->getHeader()-> getNexID();
        p3->getHeader()->setNexID(temp_Pre);
        p3->getHeader()->setPreID(temp_Nex); //pre和nex對調
        send_handler (p3);
...
void addEdge(unsigned int u,unsigned int v,unsigned int o_w ,unsigned int n_w)
{
    //cout << "fuck_fuck";
	vertex[u].push_back(v); //建立adjacency_matrix
	vertex[v].push_back(u);
	old_weight[u][v] = o_w;  //記錄舊的權重
	old_weight[v][u] = o_w;
	new_weight[u][v] = n_w;  //記錄新的權重
	new_weight[v][u] = n_w;
}

void shortestPath(int n, int src, int codi) //codi判斷做新(0)或舊(1)的dijkatra
{
    priority_queue< iPair, vector <iPair> , greater<iPair> > pq; //前面存放起點到該點的路徑長 放面存該點的編號
    vector<int> dist(n, INF); //紀錄起點到該點的距離陣列

    pq.push(make_pair(0, src)); //初始化出發點路徑長為0與該點的元素
	dist[src] = 0;
	int prev[10000];
	...
	while (!pq.empty())
	{
        ...
        for (auto i:vertex[u])  //遍歷vertex裡面的元素並且取出點和權重 想問要怎麼改比較好
		{
            int Weight;
            if(codi == 0) Weight = old_weight[i][u];
            if(codi == 1) Weight = new_weight[i][u];
            if (dist[i] == dist[u] + Weight && prev[i] > u) //如果有長度相同的路徑就選擇編號小的那條
			{
				prev[i] = u;
            }
            if (dist[i] > dist[u] + Weight)  //dijlstra的核心步驟
			{
				dist[i] = dist[u] + Weight;
				pq.push(make_pair(dist[i], i));
				prev[i] = u; //紀錄當前點的上一位元素是誰
            }
		}
	...
	for(int i = 0; i < n; i++){
        if(codi == 0){
            old_table[i][src] = prev[i]; //第一個是起點 src是終點 存放起點的下一個點
            //mat.push_back(i,prev[i]) {1};
        }
        else{
            new_table[i][src] = prev[i]; //第一個是起點 src是終點 存放起點的下一個點
            //cout<<new_table[0][4]<<"hu"<<"\n";
            //cout<<new_table[4][0]<<"fu"<<"\n";
            if(i == src){
                continue;
            }
            mat[i].push_back(prev[i]);
            mat[prev[i]].push_back(i);
            mat[i][prev[i]] = 1;
            mat[prev[i]][i] = 1;
        }
    }
...
class times{
        public:
            int Time, SrcID, DstID;
    }t[10000];

class ctrl_data{   //存放ctrl和data packet的空間
    public:
        int mode, id_ctrl, dst_ctrl, t;//mode0是ctrl 1是data
        string text_ctrl;
...
void bfs(int p,int q)
{
    ...
    while(!que.empty()){
        ...
        que.pop();
        for(int i=0;i<dot;i++){
            if(mat[a][i]==1 && vis[i]==0){
                distance[i] = distance[a] + 1;
                vis[i] = 1;
                //rps[a].push_back(i);
                que.push(i);
            }
            if(mat[a][i]==1 &&vis[i]==1 && i==q){
                rps[p][i] = distance[i];
                //rps[a][i] = distance[i]; //將第i點到a點的距離存入rps裡面
                //cout<<q<<" " << rps[a][i] << '\n';
                //cout<<"q:"<<q<<" " << rps[p][i] << '\n';
                //cout<<q<< rps[p][i] << '\n';
                //cout<< rps[p][i] << '\n';
...
int main()
{
    ...
    for (unsigned int id = 0; id < dot; id ++){
    ...
    for(int i=0;i<Link;i++){
        cin >> n[i].LinkId >> n[i].Node1 >> n[i].Node2 >> n[i].oldW >> n[i].newW;
        addEdge(n[i].Node1, n[i].Node2, n[i].oldW,n[i].newW);
        node::id_to_node(n[i].Node1)->add_phy_neighbor(n[i].Node2);
        node::id_to_node(n[i].Node2)->add_phy_neighbor(n[i].Node1);
    }

    int temp = 0;
    while(cin >> t[temp].Time){
        cin >> t[temp].SrcID >> t[temp].DstID;
        temp++;
    }

    for(int i=0;i<Dest;i++){  //根據有幾個終點座標進行幾次dijkstra
        shortestPath(dot, DestId[i], 0);
        shortestPath(dot, DestId[i], 1);
	}
    ...
    for(int i=0;i<dot;i++){
        ...
        for(int j=0;j<Dest;j++){
            if(i==DestId[j]) continue; //如果終點和起點相同就不需要輸出了
            cd[cont].id_ctrl = i;
            cd[cont].dst_ctrl = DestId[j];
            cd[cont].t = InsTime;
            cd[cont].text_ctrl = to_string(DestId[j]) + " " + to_string(old_table[i][DestId[j]]);
            cd[cont].mode = 0;
            ctrl_packet_event(con_id, i, DestId[j], old_table[i][DestId[j]], InsTime); //送出還沒更新的ctrl_packet
            //cout<<"hey\n";
            cont++;
            //cout << DestId[j] << " " << old_table[i][DestId[j]] << "\n"; //其餘的依照格式輸出
        }
        /*for(int id=0;id<node::getNodeNum();id++){
            ctrl_packet_event(con_id, cd[id].id_ctrl, id, id+1, InsTime);
        }
        */
    }
    ...
    //cout << "xxxxxxxxxxx" << '\n';
    for(int i=0;i<dot;i++){
        if(old_table[i] != new_table[i]){  //有更改才會輸出更新後的資料
            //cout << i << "\n";
            //cd[cont].id_ctrl = i;
        }
        else{
        ...
        for(int j=0;j<Dest;j++){
            if(i==DestId[j]) continue;
            if(new_table[i][DestId[j]] != old_table[i][DestId[j]]){  //如果前一點不相同才會輸出
                cd[cont].id_ctrl = i;
                cd[cont].dst_ctrl = DestId[j];
                cd[cont].t = UpdTime;
                cd[cont].text_ctrl = to_string(DestId[j]) + " " + to_string(new_table[i][DestId[j]]);
                cd[cont].mode = 0;
                cont++;
                //ctrl_packet_event(con_id, i, DestId[j], new_table[i][DestId[j]], UpdTime);
            }//cout << DestId[j] << " " << new_table[i][DestId[j]] << "\n";
        }
    }
    for(int i=0;i<Dest;i++){
        for(int j=0;j<dot;j++){
            if(rps[DestId[i]][j] == 1){
                ctrl_packet_event(node::getNodeNum()-1, i, DestId[j], new_table[i][DestId[j]],UpdTime); //根據bfs結果依序送出封包
            }
        }
    }
    ...
    for(int i=0;i<temp;i++){
        cd[cont].src_data = t[i].SrcID;
        cd[cont].dst_data = t[i].DstID;
        cd[cont].t = t[i].Time;
        //cd[cont].text_data = to_string(t[i].DstID) + " " + to_string(new_table[t[i].SrcID][t[i].DstID]);
        cd[cont].mode = 1;
        cont++;
    }
    ...
    for(int i=0;i<DestId.size();i++){
        for(int j=0;j<dot;j++){
            bfs(DestId[i], j); //進行bfs並存入rps裡面
        }
    }
    // cout << "ppppppppppp" << '\n';
    //qsort(rps,100,sizeof(rps[0]),cmp); //依照距離終點長度排序 小到大
    //sort(rps.begin(),rps.end());


    //sort(cd, cd+cont, mycompare);
    ...
    for(int i=0;i<cont;i++){  //依照排序結果傳入packet
        if(cd[i].mode == 0){
            //ctrl_packet_event (cd[i].id_ctrl, cd[i].t, cd[i].text_ctrl);
            continue;
        }
        else{
            //cout<<"hi\n";
            data_packet_event (cd[i].src_data, cd[i].dst_data, cd[i].t); //送出data_packet
        }
    }
