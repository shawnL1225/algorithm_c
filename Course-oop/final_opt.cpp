...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet" ) { // the switch receives a packet from the other switch
        ...
        // cout << "node " << getNodeID() << " send the packet " << " to :" << next_table[dst] << endl;
        send_handler (p2);
    }
...
usint nodesNum, nodeCost, linkNum;
usint InsTime, UpdTime, SimDuration;
usint srcID, dstID, pathLen;
class Node {
  public:
  int id;
  int next_new;
...
vector<Link> links;
vector<Node> nodes;
map<usint, usint> oldPathMap;
vector<usint> oldPath;
vector<int> pathOrder;
vector<int> parentOrder;
vector<pair<usint,usint>> updNodes,newNodes, delNodes;
int totalCost = 0;
map<pair<usint, usint>,int> linkweight;
void SDN_controller::recv_handler (packet *p){
    ...
    if(p->type() == "SDN_ctrl_new_packet"){
        ...
        //     state[src]++;
        //     if(state[src] == 2) return;
        //     for(auto x : newNodes){
        //         if(state[src] == 3) return;
        //     }
        //     if( !updNodes.empty()) {
        //         ctrl_upd_packet_event(nodesNum, updNodes.back().first, dstID, updNodes.back().second);
        //         updNodes.pop_back();
        //     }else{
        //         for(auto x : delNodes){
        //             ctrl_del_packet_event(nodesNum, x, dstID, x);
        //         }
        //     }
        // }
    }
        ...
        int dst = p3->getHeader()->getDstID();
        int src = p3->getHeader()->getSrcID();
        map<usint,usint> link_info = l3->getLinkWeight();
        for(auto link : link_info){
            ...
            // cout << src << ' '<<link.first <<' '<< link.second << endl;
            linkweight[{src,link.first}] = link.second;
            linkweight[{link.first, src}] = link.second;
        }
        recv_stat[src] ++;
        if(true){ //for update
            ...
            for(int k=0;k<nodesNum-1;k++){
                for(auto l :links){
                    int cost = 0;
                    if(pathOrder[l.node2] > 0){
                        cost =  nodeCost*abs(parentOrder[l.node2] - parentOrder[l.node1]);
                    }
                    else if(pathOrder[l.node2] == 0){
                        cost = nodeCost;
                    }
                    
                    if(dis[l.node2] > dis[l.node1] + l.neww + cost){
                        dis[l.node2] = dis[l.node1] + l.neww + cost;
                        nodes[l.node2].next_new = l.node1; //upadate parent (routing table)

                        if(pathOrder[l.node2] > 0){
                            int root = parentOrder[l.node1];
                            auto it = oldPath.begin();
                            while(*it != l.node2) it++;
                            int newC = pathOrder[*it];
                            ...
                            parentOrder[l.node2] = parentOrder[l.node1];
                        }
                        
                    }
                    // else if(dis[l.node2] == dis[l.node1] + l.neww + cost){ 
                    //     if(nodes[l.node2].next_new > l.node1){


                    //         nodes[l.node2].next_new = l.node1;
                    //     }
                    // }
                }
            }
            
            //check node update color
            // cout << "------------the path-------\n";
            int current = srcID;
            while(current != dstID){
                if(oldPathMap.count(current)){  //update
                     if(nodes[current].next_new != oldPathMap[current]){
                        updNodes.push_back(make_pair(current, nodes[current].next_new));
                        // cout << "u" << current << ' ';
                    }
                    oldPathMap.erase(current);
                }
                else{ //new
                    newNodes.push_back(make_pair(current, nodes[current].next_new ));
                    // ctrl_new_packet_event(nodesNum, current, dstID, nodes[current].next_new);
                }
                current = nodes[current].next_new;
                totalCost += linkweight[{current , nodes[current].next_new}];
            }
           

            for(auto x : oldPath){
                if(oldPathMap.count(x)){
                     delNodes.push_back({x, oldPathMap[x]});
                    //  cout << x << oldPathMap[x]<<endl;
                }
            ...
            // }
            int t=event::getCurTime();
            for(auto it = newNodes.rbegin();it!=newNodes.rend();it++){
                auto v_temp = *it;
                ctrl_new_packet_event(nodesNum, v_temp.first, dstID,v_temp.second,t);
                totalCost += nodeCost;
                t+=10;
            }
            for(auto it = updNodes.rbegin();it!=updNodes.rend();it++){
                auto v_temp = *it;
                ctrl_upd_packet_event(nodesNum, v_temp.first, dstID,v_temp.second,t);
                totalCost += nodeCost;
                t+=10;
            }
            for(auto it = delNodes.begin();it!=delNodes.end();it++){
                auto v_temp = *it;
                ctrl_del_packet_event(nodesNum, v_temp.first, dstID, v_temp.second, t);
                totalCost += nodeCost;
                t+=10;
            }
            if(totalCost >1250)
                cout << "===========t "<<totalCost << endl;
        }
...
int main()
{
    ...
    cin >> nodesNum >> nodeCost;
    cin >> InsTime >> UpdTime >> SimDuration;
    cin >> srcID >> dstID >> pathLen;
    ...
    //install ctrl (path)
    usint node1, node2, nid;
    pathOrder.resize(nodesNum);
    parentOrder.resize(nodesNum);
    for(int i = 0; i<pathLen; i++){
        cin >> nid >> node1 >> node2;
        oldPathMap[node1] = node2;
        oldPath.push_back(node1);
        pathOrder[node1] = pathLen-i;
        parentOrder[node1] = pathLen-i;
        ctrl_new_packet_event(con_id, node1, dstID, node2, InsTime);
    }
    //send  state (link&weight)
    map<usint,usint> link_info_new[nodesNum];
    usint lid, weight;
    cin >> linkNum;
    for(int i=0; i<linkNum; i++){
        cin >> lid >> node1 >> node2 >> weight;
        node::id_to_node(node1)->add_phy_neighbor(node2);
        node::id_to_node(node2)->add_phy_neighbor(node1);
        link_info_new[node1][node2] = weight;
        link_info_new[node2][node1] = weight;
    }
     for(int i=0; i<nodesNum; i++){
        stat_packet_event(i, con_id, link_info_new[i], UpdTime);
    }
